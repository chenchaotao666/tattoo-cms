...
import { translateText, translateRichText } from '@/services/translation';
import { ProFormItem } from '@ant-design/pro-components';
import SimpleQuill from '@/components/RichTextEditor/SimpleQuill';
...
interface FieldConfig {
  ...
  required?: boolean;
  rows?: number;
  height?: number; // For richtext fields
}

interface RichTextLanguageFormProps {
  fields: FieldConfig[];
...
const RichTextLanguageForm: React.FC<RichTextLanguageFormProps> = ({
  fields,
  activeLanguages,
  onLanguagesChange,
  formRef,
    ...
    try {
      ...
      for (const lang of targetLanguages) {
        ...
        for (const field of fieldsToTranslate) {
          ...
          try {
            let translatedText;
            const originalText = baseFields[field.key];

            if (field.type === 'richtext') {
              // 对于富文本内容，保留HTML结构进行翻译
              translatedText = await translateRichText(originalText, 'en', lang);
            } else {
              // 普通文本直接翻译
              translatedText = await translateText(originalText, 'en', lang);
            }
    ...
    try {
      ...
      for (const field of fieldsToTranslate) {
        ...
        try {
          ...
          if (field.type === 'richtext') {
            // 对于富文本内容，保留HTML结构进行翻译
            translatedText = await translateRichText(originalText, 'en', targetLang);
          } else {
            // 普通文本直接翻译
            translatedText = await translateText(originalText, 'en', targetLang);
          }
...
export default RichTextLanguageForm;